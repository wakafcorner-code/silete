...
export default function BankPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [aRes, tRes] = await Promise.all([
        ...
        fetch("/silete/api/finance/bank-transactions?limit=50"),
      ]);
  ...
  const handleCreateAccount = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/silete/api/finance/bank-accounts", {
        method: "POST",
  ...
  const handleRecordTx = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/silete/api/finance/bank-transactions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
