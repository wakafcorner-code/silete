...
import { listSalesOrders, createSalesOrder, SalesOrderSchema } from "@/services/sales-order-service";
  ...
  try {
    ...
    const result = await listSalesOrders(
      session,
      {
        ...
        status: (searchParams.get("status") ?? undefined) as SalesOrderStatus | "all" | undefined,
        customerId: searchParams.get("customer_id") ? Number(searchParams.get("customer_id")) : undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await createSalesOrder(session, parsed.data, body.company_id);
