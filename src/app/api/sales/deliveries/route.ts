...
import { listDeliveries, createDelivery, DeliverySchema } from "@/services/delivery-service";
  ...
  try {
    ...
    const result = await listDeliveries(
      session,
      {
        ...
        status: (searchParams.get("status") ?? undefined) as DeliveryStatus | "all" | undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await createDelivery(session, parsed.data, body.company_id);
