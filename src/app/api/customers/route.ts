...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await listCustomers(session, { page, limit, search, status }, companyId);
    return NextResponse.json({ success: true, ...result });
  } catch (error) {
    const status = (error as Error & { statusCode?: number }).statusCode || 500;
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Error listing customers" },
      { status }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const customer = await createCustomer(session, body, body.company_id);
